...
HORD_SCOPE_CLASS::operator()(
	...
	if (new_slug.empty()) {
		...
		auto const* const parent = datastore.find_ptr(object.parent());
		auto const& parent_children = parent ? parent->children() : datastore.root_objects();
		for (auto const child_id : parent_children) {
			auto const* const child = datastore.find_ptr(child_id);
			if (child && child->slug_hash() == slug_hash) {
